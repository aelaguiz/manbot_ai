...
from ai.lib.loaders.epub_loader import EPubLoader
...
def main():
    ...
    loader = DirectoryLoader(books_directory, glob="**/*.epub", show_progress=True, loader_cls=EPubLoader, use_multithreading=True)
    docs = loader.load()
    ...
    for doc in all_docs:
        ...
        print(f"Adding document title:{metadata['title']} author:{metadata['author']} start_index:{metadata['start_index']} {len(text)} first 50 chars: {text[:50]}")
